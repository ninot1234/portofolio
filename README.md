...
<head>
  ...
  <title>Portfolio — Alfino Daiva Herlambang </title>
  <meta name="description" content="Modern personal portfolio dengan animasi pada setiap toggle" />
  ...
  <style>
    ...
    *{box-sizing:border-box}
    ...
    body{
      ...
      background: radial-gradient(1000px 600px at 10% 10%, rgba(124,58,237,0.08), transparent),
  ...
  box-shadow: 0 6px 18px rgba(12, 18, 30, 0.6); /* Memberikan efek bayangan */
    ...
    .chev svg{width:18px;height:18px;transform-origin:center}
...
<body>
  ...
  <script>
    // Helper: smooth height animation using Web Animations API
    function animateHeight(el, toHeight){
      const from = el.getBoundingClientRect().height;
      const anim = el.animate([
        {height: from + 'px', opacity: getComputedStyle(el).opacity},
        {height: toHeight + 'px', opacity: toHeight>0 ? 1 : 0}
      ],{duration:300,easing:'cubic-bezier(.2,.9,.2,1)'});
      anim.onfinish = ()=>{
        el.style.height = toHeight + 'px';
        if(toHeight===0) el.style.display = 'none';
    ...
    document.querySelectorAll('.toggle').forEach(section=>{
      ...
      head.addEventListener('click', ()=>{
        ...
        if(isOpen){
          ...
          // open
          // set display block to measure
          content.style.display = 'block';
          const targetH = content.scrollHeight;
          content.style.height = getComputedStyle(content).height; // lock current
          content.classList.add('open');
          chev.style.transform = 'rotate(180deg)';
          animateHeight(content, targetH);
        }
      });
    });

    // NAV buttons change scroll/focus and open corresponding toggle
    document.querySelectorAll('.nav-btn').forEach(btn=>{
      btn.addEventListener('click', ()=>{
        ...
        // close others
        document.querySelectorAll('.toggle').forEach(s=>{
          const c = s.querySelector('.content');
          const ch = s.querySelector('.chev');
          if(s.id!==target && c.classList.contains('open')){
